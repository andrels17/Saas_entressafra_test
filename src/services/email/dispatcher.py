    ...
    if not eq_prog.empty:
        for _, row in eq_prog.iterrows():
            ...
            for t in tasks:
                ...
                # IMPORTANTE:
                # `updated_at` em tarefas pendentes/estruturais não pode contar como
                # movimentação, senão a página "Equipamentos sem movimentação"
                # zera tudo indevidamente. Só consideramos `updated_at` como
                # fallback quando já houve atividade real no item.
                mov_ts = _best_ts(
                    t.get("dt_etapa_m"),
                    t.get("dt_etapa_r"),
                    t.get("dt_etapa_d"),
