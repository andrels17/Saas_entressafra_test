...
class RelatorioExecutivoPayload:
    ...
    heatmap_semanal: list[dict] | None = None        # [{departamento, semana, pct}]
    alertas_parados: dict | None = None              # {atencao, critico, urgente}
...
def build_executive_pdf(payload: RelatorioExecutivoPayload) -> bytes:
    ...
    def pbar(x, y, bw, bh, pct, color=None):
        ...
        fw = max(bw * max(0, min(int(pct or 0), 100)) / 100, bh if pct and pct > 0 else 0)
        if fw > 0:
            c.setFillColor(col)
    ...
    def mini_kpi_row(y_top: float, items: list[tuple[str, str, colors.Color]], cols: int = 4) -> float:
        card_h = 16*mm
        gap = 2*mm
        ...
        for i, (lbl, val, col) in enumerate(items):
            ...
            c.setLineWidth(0.6)
            c.roundRect(cx, y_top - card_h, card_w, card_h, 3, fill=1, stroke=1)
            c.setFillColor(col)
    ...
    deptos = sorted(payload.departamentos, key=lambda d: (-int(d.pct_geral or 0), d.nome))
    n_deptos = len(deptos)
    ...
    top_atrasados = sorted(deptos, key=lambda d: (int(d.pct_geral or 0), -int(d.n_travados or 0), d.nome))[:5]
    top_evolucao = sorted(deptos, key=lambda d: (int(d.pct_geral or 0) - int(d.pct_anterior or 0), int(d.pct_geral or 0)), reverse=True)[:5]
    ...
    if alertas_cfg:
        ...
        n_urgente = sum(int(getattr(d, 'n_parados', 0) or 0) for d in deptos if int(getattr(d, 'max_dias_parado', 0) or 0) > 21)
        n_critico = sum(int(getattr(d, 'n_parados', 0) or 0) for d in deptos if 14 < int(getattr(d, 'max_dias_parado', 0) or 0) <= 21)
        n_atencao = sum(int(getattr(d, 'n_parados', 0) or 0) for d in deptos if 7 < int(getattr(d, 'max_dias_parado', 0) or 0) <= 14)
    ...
    for i, dept in enumerate(deptos):
        if y - row_h < 18*mm:
            footer(); c.showPage(); page_header('Ranking de Departamentos (cont.)'); y = h - 20*mm
        ...
        c.drawString(32*mm, y - row_h*0.72, f"{dept.n_equipamentos} equip · {dept.n_concluidos} OK · {dept.n_travados} travados · {dept.n_sem_inicio} sem início")
        bar_x = w/2
    ...
    box_bottom = draw_top5_box(left_x, y, col_w, 'Top 5 evolução da semana', top_evolucao, mode='delta')
    draw_top5_box(right_x, y, col_w, 'Top 5 mais atrasados', top_atrasados, mode='pct')
    y = box_bottom - 8*mm
    ...
    c.setFillColor(SURFACE)
    c.setStrokeColor(BORDER)
    c.roundRect(16*mm, y - chart_h, w - 32*mm, chart_h, 3, fill=1, stroke=1)
    ...
    if trend:
        ...
        for i, row in enumerate(trend):
            ...
            pts.append((px, py, int(row.get('semana', 0)), int(row.get('pct', 0))))
        ...
        c.setFillColor(MUTED)
        c.setFont('Helvetica', 8)
        c.drawCentredString(16*mm + (w - 32*mm)/2, y - chart_h/2, 'Adicione payload.trend_semanal para mostrar as últimas 4 semanas.')
    y -= chart_h + 8*mm
    ...
    y = mini_kpi_row(y, [
        ...
        ('Maior tempo parado', f"{maior_dias}d" if maior_dias else '0d', _stoppage_color(maior_dias)),
    ], cols=3)
    ...
    for i, dept in enumerate(sorted(linhas, key=lambda d: (-int(getattr(d, 'n_parados', 0) or 0), -int(getattr(d, 'max_dias_parado', 0) or 0), d.nome))):
        if y - row_h < 18*mm:
            footer(); c.showPage(); page_header('Equipamentos sem movimentação (cont.)'); y = h - 20*mm
        ...
        pbar(164*mm, y - row_h + 2.4*mm, 23*mm, 5*mm, round((n_par / max(max_parados, 1)) * 100), color=bar_col)
    ...
    if heatmap:
        ...
        for i, sem in enumerate(semanas):
            c.setFillColor(MUTED)
            c.setFont('Helvetica-Bold', 7.2)
            c.drawCentredString(table_x + i*cell_w + cell_w/2, y, f"Sem.{sem}")
        y -= 5*mm
        data = {(str(r.get('departamento', '')), int(r.get('semana', 0))): int(r.get('pct', 0) or 0) for r in heatmap}
        for dept_name in dept_names:
            if y - cell_h < 18*mm:
                footer(); c.showPage(); page_header('Heatmap de manutenção por departamento (cont.)'); y = h - 20*mm
            c.setFillColor(FG)
            c.setFont('Helvetica-Bold', 7)
            ...
            for i, sem in enumerate(semanas):
                ...
                c.setFillColor(fill)
                c.roundRect(table_x + i*cell_w, y - cell_h, cell_w - 2, cell_h - 1, 2, fill=1, stroke=0)
                c.setFillColor(WHITE)
                c.setFont('Helvetica-Bold', 7)
                c.drawCentredString(table_x + i*cell_w + (cell_w - 2)/2, y - 5.5*mm, f"{pct}%")
            y -= cell_h + 1.5*mm
        footer(); c.showPage()
    ...
    def dept_block_height(dept: DeptSnapshot) -> float:
        ...
        row_gap = 6.5 * mm
        bottom_pad = 6 * mm
        sections = [min(len(dept.top_criticos), 3), min(len(dept.top_melhores), 3), min(len(dept.maiores_evolucoes), 3)]
    ...
    def draw_dept_block(dept: DeptSnapshot, bx: float, by: float, bh: float):
        ...
        if dept.top_melhores:
            ...
            for eq in dept.top_melhores[:3]:
                ...
                c.setFont('Helvetica', 7)
                c.drawString(bx + 20*mm, iy, str(eq.get('modelo') or '')[:13])
                pbar(bx + col_w - 26*mm, iy - 1.5*mm, 18*mm, 3.5*mm, pct_eq, color=GREEN if pct_eq >= 80 else YELLOW)
                c.setFillColor(GREEN)
                c.setFont('Helvetica-Bold', 7.5)
                c.drawRightString(bx + col_w - 4, iy, f"{pct_eq}%")
                iy -= 6*mm
    ...
    for dept in deptos:
        ...
        for _ in range(3):
            ...
            if y_col[next_ci] - bh >= PAGE_BOT:
                ...
                footer(); c.showPage(); page_header('Destaques por Departamento (cont.)'); y_col = [PAGE_TOP, PAGE_TOP]; ci = 0
        if not placed:
            draw_dept_block(dept, col_x[ci], y_col[ci], bh)
            y_col[ci] -= bh + 3*mm
            ci = 1 - ci
