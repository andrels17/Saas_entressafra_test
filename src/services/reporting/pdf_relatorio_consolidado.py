...
class DeptSummary:
    ...
    # lista de EquipamentoCritico
    criticos: list = field(default_factory=list)
...
class RelatorioConsolidadoPayload:
    ...
    def pct_geral(self) -> int:
        if not self.departamentos:
            return 0
        return round(sum(d.pct_geral for d in self.departamentos) /
...
def build_consolidated_pdf(payload: RelatorioConsolidadoPayload) -> bytes:
    ...
    def kpi_row(items, top_y: float, card_h: float = 20 * mm) -> float:
        ...
        for i, (label, val, val_color) in enumerate(items):
            ...
            c.setFillColor(PRIMARY)
            c.rect(cx, top_y - 2.5, card_w, 2.5, fill=1, stroke=0)
            c.setFillColor(MUTED)
            c.setFont("Helvetica", 8)
    ...
    for dept in depts_sorted:
        ...
        progress_bar(
            ...
            row_h - 3 * mm,
            dept.pct_geral)
        c.setFillColor(_risk_color(dept.pct_geral))
        c.setFont("Helvetica-Bold", 8.5)
    ...
    rows2 = [["Departamento", "🚫 Travados",
              "⬜ Sem início", "⏸ Parados", "⚠️ Risco prazo"]]
    has_alerts = False
    for dept in depts_sorted:
        total_a = dept.n_travados + dept.n_sem_inicio + \
            dept.n_parados + dept.n_risco_prazo
        if total_a == 0:
            continue
        has_alerts = True
        rows2.append([
    ...
    if has_alerts:
        y = platypus_table(
            rows2, [
                pw * 0.36, pw * 0.16, pw * 0.16, pw * 0.16, pw * 0.16], y)
    else:
        ...
        c.drawString(
            16 * mm,
            y - 8 * mm,
            "✓ Nenhum alerta ativo em todos os departamentos.")
    ...
    for dept in depts_sorted:
        ...
        if dept.criticos:
            y = section_title(
                f"Equipamentos críticos ({len(dept.criticos)})", y)
            status_labels = {
                "zero": "0% — Sem início",
                "travado": "Travado",
                "atrasado": "Atrasado"}
        ...
        if n_with_alerts:
            ...
            for label, count, col in alert_items:
                ...
                if y < 30 * mm:
                    break
                c.setFillColor(col if count else MUTED)
                c.setFont("Helvetica-Bold", 9)
