...
def _hex_to_color(
        hex_str: str,
        fallback: colors.Color = colors.red) -> colors.Color:
    try:
        s = (hex_str or "").strip()
        if not s:
            return fallback
...
def build_executive_pdf(
    ...
    def footer(page_no: int):
        ...
        c.drawRightString(w - 16 * mm, 11 * mm,
                          f"{fmt_brt()}  •  pág. {page_no}")
    ...
    def mini_bars(
            title: str, items: List[Dict[str, Any]], top_left: Tuple[float, float]):
    ...
    y_after = mini_bars("Distribuição por Status",
                        dist, (16 * mm, h - 98 * mm))
    ...
    for it in top_serv[:8]:
        serv_rows.append([str(it.get("servico_nome", "-"))
                         [:42], int(it.get("count", 0))])
    simple_table(
        "Top Serviços", serv_rows, [
            90 * mm, 25 * mm], (16 * mm, y_after))
    ...
    for it in top_set[:10]:
        set_rows.append([str(it.get("setor_nome", "-"))
                        [:42], int(it.get("count", 0))])
    y = simple_table(
        "Top Setores", set_rows, [
            90 * mm, 25 * mm], (16 * mm, h - 50 * mm))
    ...
    simple_table("Itens Críticos (amostra)", crit_rows, [
                 32 * mm, 25 * mm, 70 * mm, 25 * mm], (16 * mm, y))
