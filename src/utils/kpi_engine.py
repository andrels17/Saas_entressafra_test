        ...
        if not trows:
            ...
            while True:
                ...
                if len(page) < page_size:
                    break
                start += page_size
            if not trows and _fallback_error is not None:
                log.warning(
                    "KPI ZERADO — RPC e query direta falharam em tarefas_servico. "
                    "Crie a funcao get_tarefas_kpi_dashboard no Supabase. "
                    "Erro: %s", _fallback_error
                )
