...
def is_mobile() -> bool:
    ...
    try:
        qp = st.query_params
        qp_mobile = str(qp.get("mobile", "")).lower() in ("1", "true", "yes", "on")
    except Exception:
        qp_mobile = False
...
def render_mobile_toggle():
    ...
    def _sync_mobile_from(widget_key: str):
        ...
        st.session_state["__force_mobile__"] = bool(st.session_state.get(widget_key, False))
    ...
    # Logout no corpo: apenas em mobile (evita redundância com a sidebar no desktop)
    with cols[1]:
        if is_mobile() and st.session_state.get("sb_access_token"):
            if st.button("⎋ Sair", key="body_logout", use_container_width=True):
                try:
