...
def load_branding(sb, tenant_id: str) -> Dict[str, Any]:
    res = sb.table("tenant_branding").select(
...
def save_branding(sb, tenant_id: str, payload: Dict[str, Any]) -> None:
    sb.table("tenant_branding").upsert(
        {**payload, "tenant_id": tenant_id}).execute()
...
def hex_to_rgb(hex_color: str) -> Tuple[float, float, float]:
    ...
    if len(h) != 6:
        return (0.9, 0.9, 0.9)
    return (int(h[0:2], 16) / 255.0, int(h[2:4], 16) /
            255.0, int(h[4:6], 16) / 255.0)
...
def build_executive_pdf(
    ...
    total = safe_int(kpis.get("total") or summary.get(
        "total") or summary.get("total_registros"))
    concl = safe_int(kpis.get("concluidos") or kpis.get(
        "finalizados") or summary.get("concluidos"))
    ...
    for label, val in [("Total", total), ("Concluídos", concl),
                       ("Pendentes", pend), ("Travados", trav)]:
        if val:
    ...
    story += [Spacer(1, 0.8 * cm),
              Paragraph(branding.footer_note, small), PageBreak()]

    # ── Insights ────────────────────────────────────────────────────────────
    story.append(Paragraph("Principais insights", h1))
    insights: List[str] = []

    backend_insights = summary.get(
        "insights") or summary.get("principais_insights")
    if isinstance(backend_insights, list):
        for it in backend_insights:
            txt = it.strip() if isinstance(it, str) else (
                it.get("text") or it.get("insight") or "").strip()
            if txt:
                insights.append(txt)

    comp = summary.get("comparativos") or summary.get("deltas") or {}
    for campo, label in [("travados", "Travados"),
                         ("concluidos", "Concluídos")]:
        old = comp.get(f"{campo}_anterior") or comp.get(f"{campo}_prev")
        new = comp.get(f"{campo}_atual") or comp.get(
            f"{campo}_curr") or locals().get(campo[:4])
        if old is not None and new is not None:
            ch = pct_change(safe_float(new), safe_float(old))
            if ch is not None:
                insights.append(
                    f"{label} {
                        'subiram' if ch >= 0 else 'caíram'} {
                        abs(ch):.1f}% no período.")
    ...
    if total_t > 0:
        insights.append(
            f"Conclusão no período: {
                int(concl_t)}/{
                int(total_t)} ({
                concl_t / total_t * 100:.1f}%).")
        insights.append(
            "Nenhuma tarefa travada no período." if trav_t == 0 else f"Tarefas travadas: {
                int(trav_t)} ({
                trav_t /
                total_t *
                100:.1f}%).")
        ...
        if isinstance(dist, list):
            pend_item = next(
                (d.get("count") for d in dist if isinstance(
                    d,
                    dict) and str(
                    d.get(
                        "status",
    ...
    for key, name_keys, label in [
        ("top_servicos", [
            "servico_nome", "nome", "label", "servico"], "Serviço"), ("top_setores", [
            "setor_nome", "nome", "label", "setor"], "Setor"), ("top_grupos", [
                "grupo_nome", "nome", "label", "grupo"], "Grupo"), ]:
        best = _best_by_pct(summary.get(key), name_keys)
        if best and best[1] > 0:
            insights.append(
                f"{label} com melhor taxa de conclusão: <b>{best[0]}</b> ({best[1] * 100:.1f}%).")

    if not insights:
        insights.append(
            "Sem insights automáticos disponíveis para este recorte.")
            ...
            if isinstance(it, dict):
                lbl = next(
                    (str(
                        ...
                        "grupo"] if it.get(k)),
                    "").strip()
                qtd = safe_float(it.get("qtd") or it.get(
                    "quantidade") or it.get("count"), 0.0)
                pctd = safe_float(it.get("pct_done") or it.get(
                    "percent_done") or it.get("pct_concluido") or it.get("pct"), 0.0)
                if lbl:
                    labels_out.append(lbl)
                    qtds.append(qtd)
                    pcts.append(pctd)
            elif isinstance(it, (list, tuple)) and len(it) >= 2:
                labels_out.append(str(it[0]))
                qtds.append(safe_float(it[1]))
                pcts.append(safe_float(it[2]) if len(it) >= 3 else 0.0)
        return labels_out, qtds, pcts

    charts_block: List[Any] = []
    for key, section_title in [("top_servicos", "Top Serviços"),
                               ("top_setores", "Top Setores"), ("top_grupos", "Top Grupos")]:
        lbls, qtds, pcts = _extract_top(key)
        if lbls and qtds:
            charts_block.append(Spacer(1, 0.35 * cm))
            charts_block.append(Paragraph(section_title, h2))
            png = make_bar_chart_png(
                lbls, qtds, f"{section_title} (quantidade)")
            if png:
                charts_block.append(
                    Image(
                        _io.BytesIO(png),
            ...
            if show_percent and any(pcts):
                png2 = make_bar_chart_png(lbls,
                                          [v * 100 for v in pcts],
                                          f"{section_title} (% concluído)",
                                          ylabel="%")
                if png2:
                    charts_block += [Spacer(1,
                                            0.2 * cm),
                                     Image(_io.BytesIO(png2),
                                           width=16.2 * cm,
                                           height=7.0 * cm)]
    ...
    if isinstance(weekly, list) and weekly:
        ...
        if non_null == 0 and len(labels_w) == 1:
            ...
            charts_block.append(Paragraph(
                f"Sem semana definida: <b>{done}/{tot}</b> concluídas (<b>{pct_:.1f}%</b>).", p))
        else:
            pngw = make_bar_chart_png(
                labels_w,
                pct_w,
    ...
    if charts_block:
        ...
        story.append(
            Paragraph(
                "Sem dados de Top Serviços/Setores para este recorte.",
                small))
    ...
    def _on_page(canvas, _doc):
        ...
        canvas.drawRightString(
            A4[0] - 1.6 * cm, 1.0 * cm, f"{branding.company_name} • {period_label}")
        canvas.restoreState()
