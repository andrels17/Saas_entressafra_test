...
def _fragment_alertas(tenant_id: str, revisao_id: str) -> None:
    ...
    with c1:
        days_trav = st.number_input(
            "Travado há (dias)",
            min_value=1,
            max_value=60,
            value=3,
            step=1,
            key="alerta_trav_days")
    with c2:
        days_pend = st.number_input(
            "Pendente/Andamento há (dias)",
            min_value=1,
            max_value=120,
            value=7,
            step=1,
            key="alerta_pend_days")
    with c3:
        lim_alerts = st.number_input(
            "Limite alertas",
            min_value=50,
            max_value=5000,
            value=300,
    ...
    if alerts is None:
        st.warning(
            "RPC `pendencias_criticas` não encontrada. "
    ...
    travados = int((df_a.get("status") == "travado").sum()
                   ) if "status" in df_a.columns else 0
    pendentes = int((df_a.get("status") == "pendente").sum()
                    ) if "status" in df_a.columns else 0
    em_andamento = int((df_a.get("status") == "em_andamento").sum()
                       ) if "status" in df_a.columns else 0
    ...
    with m1:
        st.metric(
            "Total alertas",
            len(df_a),
            help="Registros que ultrapassaram o threshold")
    with m2:
        st.metric(
            "Travados",
            travados,
...
@st.fragment
def _fragment_parados(
        tenant_id: str,
        revisao_id: str,
        dias_sem_mov: int = 7) -> None:
    ...
    for r in rows:
        ...
        mov = max([x for x in [r.get("dt_etapa_m"), r.get("dt_etapa_r"), r.get(
            "dt_etapa_d"), r.get("updated_at")] if x] or [None])
        if mov and (item["ultima_mov"] is None or mov > item["ultima_mov"]):
            item["ultima_mov"] = mov
        sem = int(r.get("semana") or 0)
        if any([r.get("dt_etapa_d"), r.get("dt_etapa_r"), r.get(
                "dt_etapa_m")]) and sem > (item["ultima_semana"] or 0):
        ...
        out.append(
            {
                ...
                "Grupo": item["Grupo"],
                "Últ. semana": f"Sem. {
                    item['ultima_semana']}" if item.get("ultima_semana") else "Sem registro",
                "Dias sem mov.": dias,
            })

    df = pd.DataFrame(
        sorted(out, key=lambda x: (-x["Dias sem mov."], str(x["Frota"]))))
    if df.empty:
        st.success("Nenhum equipamento parado no recorte atual ✅")
        return
    m1, m2, m3 = st.columns(3)
    with m1:
        st.metric("Equipamentos parados", len(df))
    with m2:
        st.metric("Maior tempo", f"{int(df['Dias sem mov.'].max())} dias")
    with m3:
        st.metric("Grupos afetados", int(df['Grupo'].nunique()))
...
def _fragment_pendencias(
    ...
    with f1:
        grupo_nome = st.selectbox(
            "Grupo",
            _grupo_names,
            index=_grupo_names.index(_def_grupo),
            key="pend_grupo_sel")
    with f2:
        setor_nome = st.selectbox(
            "Setor",
            _setor_names,
            index=_setor_names.index(_def_setor),
            key="pend_setor_sel")
    with f3:
        busca = st.text_input(
            "Buscar equipamento",
    ...
    if grupo_nome != "(Todos)":
        st.query_params["grp"] = grupo_nome
    elif "grp" in st.query_params:
        del st.query_params["grp"]
    if setor_nome != "(Todos)":
        st.query_params["sec"] = setor_nome
    elif "sec" in st.query_params:
    ...
    if is_mobile():
        only_open = st.checkbox(
            "Somente pendentes",
            value=True,
            key="pend_mobile_open")
        default_status = [
            "travado",
            "pendente",
            "em_andamento"] if only_open else [
            "travado",
            "pendente"]
        ...
        statuses = [status_opts[status_labels.index(
            label)] for label in (sel_labels or [])]
    ...
    with st.spinner("", show_time=False):
        q = (
            sb.table("tarefas_servico") .select(
                "id,status,semana,observacao,updated_at,"
                "servicos(nome,setores(nome)),"
                "equipamentos(id,frota,modelo,equip_grupos(id,nome,departamento_id))") .eq(
                "tenant_id", tenant_id) .eq(
                "revisao_id", revisao_id))
        if statuses:
            q = q.in_("status", statuses)
        rows = q.order(
            "updated_at", desc=False).limit(
            int(limit)).execute().data or []
    ...
    for r in rows:
        ...
        if grupo_opts.get(grupo_nome) and grupo != grupo_nome:
            continue
        if setor_opts.get(setor_nome) and setor != setor_nome:
            continue
        if busca.strip() and busca.strip().lower() not in equipamento.lower():
            continue
    ...
    df["_prio"] = df["_status_raw"].map(
        {"travado": 0, "pendente": 1, "em_andamento": 2, "concluido": 3, "nao_aplica": 9}).fillna(5)
    df = df.sort_values(["_prio", "Grupo", "Equipamento",
                        "Setor", "Serviço"]).drop(columns=["_prio"])

    # Métricas do recorte atual
    m1, m2, m3 = st.columns(3)
    with m1:
        st.metric("Total no recorte", len(df))
    with m2:
        st.metric("Travados",
                  int((df["_status_raw"] == "travado").sum()),
                  delta_color="inverse")
    with m3:
        st.metric("Pendentes",
                  int((df["_status_raw"] == "pendente").sum()),
                  delta_color="inverse")
    ...
    if is_mobile():
        weeks = sorted(
            [int(w) for w in df["Semana"].dropna().unique() if str(w).isdigit()])
        wsel = st.selectbox(
            "Semana",
            weeks,
            index=0,
            key="pend_wsel") if weeks else None
        c1, c2 = st.columns(2)
        with c1:
        ...
        if confirm_dialog(
            ...
            ids = df[(df["Semana"] == wsel) & (df["_status_raw"].isin(
                ["pendente", "em_andamento"]))]["_id"].dropna().tolist()
            if ids:
                try:
                    sb.table("tarefas_servico").update(
                        {"status": "concluido"}).in_("id", ids).execute()
                    st.toast(f"{len(ids)} tarefas concluidas.",
        ...
        with c2:
            trav_reason = st.text_input(
                "Motivo padrão (travado)",
                placeholder="Ex.: aguardando peça",
                key="pend_trav_reason")

        for tab_label, stt in zip(["Travados", "Pendentes", "Em andamento"], [
                                  "travado", "pendente", "em_andamento"]):
            with st.expander(tab_label, expanded=(stt == "travado")):
                ...
                for _, row in sdf.iterrows():
                    ...
                    st.caption(
                        f"{row['Grupo']} • {row['Setor']} • Sem. {row['Semana']}")
                    ...
                    with b1:
                        if st.button("✅", key=f"done_{tid}", help="Concluir"):
                            sb.table("tarefas_servico").update(
                                {"status": "concluido"}).eq("id", tid).execute()
                            st.rerun()
                    with b2:
                        if st.button(
                                "🚧", key=f"go_{tid}", help="Em andamento"):
                            sb.table("tarefas_servico").update(
                                {"status": "em_andamento"}).eq("id", tid).execute()
                            st.rerun()
                    with b3:
            ...
            with st.expander(
                ...
                with c3:
                    motivo = st.text_input(
                        "Motivo",
                        key=f"quick_motivo_{tid}",
                        placeholder="Obrigatório para Travado")
                with c4:
                    if st.button(
                        "⛔ Travar",
                        key=f"quick_block_{tid}",
                            use_container_width=True):
                        if not motivo.strip():
...
def render_gestor_painel() -> None:
    _ph("◈", "Painel do Gestor",
        "Pendências críticas, travados e filtro rápido por grupo/setor/equipamento.")
