...
def _build_resumo_df(eqs: list[dict[str, Any]], all_services: list[dict[str, Any]], task_map: dict[tuple[str, str], dict[str, Any]], eq_label: dict[str, str]) -> tuple[pd.DataFrame, int, int, int]:
...
def build_group_context(base_ctx: MatrixBaseContext) -> MatrixGroupContext | None:
    ...
    view_agg = pd.DataFrame(view_agg_rows) if view_agg_rows else pd.DataFrame()
