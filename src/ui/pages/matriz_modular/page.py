...
def _render_altair_evo(pc: pd.DataFrame, ps: pd.DataFrame, meta: pd.Series) -> None:
    ...
    color_scale = alt.Scale(domain=['Cumulativo (%)', 'Meta (%)', 'Na semana (%)'], range=['#7F77DD', '#378ADD', '#D85A30'])
    ...
    points_highlight = base.mark_point(size=80, filled=True, opacity=0).encode(
    ...
    rule = alt.Chart(df_melt).mark_rule(color='#888780', strokeWidth=1, strokeDash=[4, 3], opacity=0.6).encode(x='Semana:Q').transform_filter(nearest)
    chart = (lines + points_highlight + rule).properties(height=320).configure_view(strokeWidth=0).configure_axis(gridOpacity=0.15, domainOpacity=0.3).configure_legend(labelFontSize=12, symbolSize=80)
...
def _render_toolbar(base_ctx):
    ...
    with hph.container():
        ...
        with row1_c1:
            ...
            search = st.text_input('Buscar', value=st.session_state['matriz_grp_search'], placeholder='Grupo ou departamento…', key='mtz_search_in')
        ...
        with row1_c3:
            st.session_state['matriz_limit_eq'] = st.number_input('Limite eq.', min_value=20, max_value=500, value=int(st.session_state['matriz_limit_eq']), step=20, key='mtz_lim_pick')

        row2_c1, row2_c2, row2_c3 = st.columns([1.05, 1.05, 1.95], vertical_alignment='bottom')
        with row2_c1:
            status_filter = st.selectbox('Status', ['Todos', '🔴 Crítico (<50%)', '🟡 Em andamento (50–79%)', '🟢 Avançado (≥80%)', '⬜ Sem dados'], index=0, key='mtz_status_filter')
        with row2_c2:
            sort_by = st.selectbox('Ordenar', ['Nome', '% ↑ (mais atrasados)', '% ↓ (mais avançados)'], index=1, key='mtz_sort_by')
        with row2_c3:
            actions_left, actions_right = st.columns([0.60, 1.40], gap='medium')
            with actions_left:
                st.session_state['matriz_show_legend'] = st.toggle('Legenda', value=bool(st.session_state['matriz_show_legend']), key='mtz_leg')
            with actions_right:
                ...
                with a1:
                    ...
                    clear_dept = st.button('🧹 Limpar', key='mtz_clear_dept', use_container_width=True, help='Remove o departamento selecionado e mantém os demais filtros.')
                    st.markdown('</div>', unsafe_allow_html=True)
                with a2:
                    st.markdown('<div class="mtz-btn-neutral">', unsafe_allow_html=True)
                    show_all = st.button('▦ Ver todos', key='mtz_show_all', use_container_width=True, help='Exibe todos os grupos novamente e limpa a busca atual.')
                    st.markdown('</div>', unsafe_allow_html=True)
                with a3:
                    st.markdown('<div class="mtz-btn-primary">', unsafe_allow_html=True)
                    reload_data = st.button('↻ Atualizar', key='mtz_reload', use_container_width=True, help='Recarrega os dados da matriz e atualiza os indicadores.')
                    st.markdown('</div>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
...
def _render_missing_services_alert(group_ctx):
    missing_svc = {
        sid for sid in [str(s.get('id')) for s in group_ctx.all_services if s.get('id')]
...
def _render_evolucao_tab(group_ctx, base_ctx):
    ...
    with col_evo1:
        rank_mode = st.radio('Escopo:', ['Grupo inteiro', 'Setor específico'], horizontal=False, key=f'evo_mode_{group_ctx.revisao_id}_{group_ctx.grupo_id}')
    setor_sel_rank = None
    with col_evo2:
        if rank_mode == 'Setor específico':
            ...
            setor_sel_rank = st.selectbox('Setor', setores_rank, index=evo_idx, key=f'evo_setor_{group_ctx.revisao_id}_{group_ctx.grupo_id}')
            st.session_state[evo_setor_key] = setor_sel_rank
        else:
            st.caption(f'Analisando **{len(group_ctx.eqs)} equipamentos** · **{len(group_ctx.all_services)} serviços** · **{len(group_ctx.all_services) * 3 * len(group_ctx.eqs)} etapas** no total')

    chosen = group_ctx.all_services if rank_mode == 'Grupo inteiro' else sorted(group_ctx.setor_to_services.get(setor_sel_rank, []), key=lambda x: (x.get('nome') or '').lower())
    seen = set()
    svc_ids_rank = []
    for s in chosen:
        sid = s.get('id')
    ...
    rev_start2 = pd.to_datetime((group_ctx.rev_row or {}).get('data_inicio') or (group_ctx.rev_row or {}).get('created_at'), errors='coerce', utc=True)
    ...
    if evo is None:
        ...
        mk2.metric('Meta (semana atual)', f'{meta_atual:.1f}%', delta=f'{delta_vs_meta:+.1f}%', delta_color='normal' if delta_vs_meta >= 0 else 'inverse')
    ...
    try:
        ...
        for rev in all_revs[:6]:
            ...
            try:
                trows = (
                    base_ctx.sb.table('tarefas_servico').select('etapa_d,etapa_r,etapa_m').eq('tenant_id', base_ctx.tenant_id).eq('revisao_id', rid).in_('equipamento_id', [e['id'] for e in group_ctx.eqs]).execute().data
                ) or []
                ...
                comp_rows.append({'Revisão': rtit, 'Status': rev.get('status') or '?', '% Concluído': round((done / total) * 100), 'Etapas concluídas': done, 'Total esperado': total})
            except Exception:
                pass
        if comp_rows:
            comp_df = pd.DataFrame(comp_rows)
            comp_df['Atual'] = comp_df.apply(lambda r: '◄ atual' if r['Revisão'] in (group_ctx.titulo, group_ctx.revisao_id) else '', axis=1)
            st.dataframe(comp_df, use_container_width=True, hide_index=True, column_config={'% Concluído': st.column_config.ProgressColumn('% Concluído', min_value=0, max_value=100)})
...
def _render_analytics_tab(group_ctx, analytics_data):
    ...
    a2.metric('Meta esperada', f"{analytics_data['expected_pct_now']:.1f}%", delta=f"{analytics_data['delta_vs_expected_now']:+.1f}%", delta_color='normal' if analytics_data['delta_vs_expected_now'] >= 0 else 'inverse')
    ...
    if group_ctx.resumo_df.empty:
        ...
        critical_equipment_df['Risco'] = critical_equipment_df['%'].apply(lambda v: 'alto' if int(v) < 50 else ('medio' if int(v) < 80 else 'baixo'))
        critical_equipment_df = critical_equipment_df.sort_values(by=['%', 'Concluidos'], ascending=[True, True]).head(10)[['Equipamento', '%', 'Concluidos', 'Total', 'Risco']]
        ...
        bar = alt.Chart(chart_df).mark_bar(height=18, cornerRadiusEnd=3).encode(
            ...
            tooltip=[alt.Tooltip('Equipamento:N', title='Equipamento'), alt.Tooltip('%:Q', title='% concluído', format='.0f'), alt.Tooltip('Risco:N', title='Risco')],
        ).properties(height=max(180, len(chart_df) * 32)).configure_view(strokeWidth=0).configure_axis(domainOpacity=0.3)
