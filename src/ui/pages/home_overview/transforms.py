...
def enrich_kdf(
    ...
    dep_scope_norm = None if dep_scope_ids is None else [str(x) for x in dep_scope_ids if x is not None]
    grp_scope_norm = None if grp_scope_ids is None else [str(x) for x in grp_scope_ids if x is not None]
    kdf["Grupo"] = kdf["grupo_id"].map(gid_to_name).fillna(kdf["grupo_id"].astype(str))
    kdf["departamento_id"] = kdf["grupo_id"].map(gid_to_dept)
    if dep_scope_norm not in (None, []):
        kdf = kdf[kdf["departamento_id"].isin(dep_scope_norm)].copy()
    if grp_scope_norm not in (None, []):
        kdf = kdf[kdf["grupo_id"].isin(grp_scope_norm)].copy()
    return kdf
