...
def _build_pdf_tables(*, titulo, grupo_nome, resumo_df, sector_tables) -> bytes:
    ...
    def _summary_table(df: pd.DataFrame):
        ...
        table = Table(rows, colWidths=[pw * 0.57, pw * 0.14, pw * 0.11, pw * 0.18], repeatRows=1)
        table.hAlign = "LEFT"
    ...
    def _build_sector_block(df: pd.DataFrame):
        ...
        equip_w = 5.2 * cm
        trio_w = 0.88 * cm
        max_groups = max(1, min(len(groups), int((pw - equip_w) // (trio_w * 3)) or 1))
        ...
        for chunk_idx, chunk in enumerate(chunks, start=1):
            ...
            for service_name, ordered_cols in chunk:
                ...
                if len(normalized) == 3:
                    top.extend([Paragraph(f"<b>{service_name}</b>", head_top), "", ""])
                    sub.extend([
            ...
            matrix_w = max(0.82 * cm, min(0.94 * cm, remaining / max(matrix_cols, 1)))
            ...
            for col in separators[:-1]:
                style_cmds.append(("LINEAFTER", (col, 0), (col, -1), 0.7, colors.HexColor("#94A3B8")))

            for row_i in range(2, len(data)):
                for col_i in range(1, len(cols_meta)):
                    bg, fg, text = _cell_heat_style(view.iloc[row_i - 2, col_i])
                    style_cmds.extend([
    ...
    for sector_name, sector_df in sector_tables:
        ...
        story.append(Paragraph(f"Geral: <b>{pct_general}%</b> | Concluídos: <b>{ok_count}/{total_cells}</b>", sector_meta_style))
