...
def render_dashboard() -> None:
    ...
    for _df_lookup in (base, equipment_base):
        if _df_lookup is None or _df_lookup.empty:
            continue
        if "grupo_id" not in _df_lookup.columns or "grupo" not in _df_lookup.columns:
            continue
        for _gid, _gname in _df_lookup[["grupo_id", "grupo"]].dropna().drop_duplicates().itertuples(index=False):
            _gid_s = str(_gid).strip() if _gid is not None else ""
            _name_s = str(_gname).strip() if _gname is not None else ""
            if not _gid_s or not _name_s or _name_s == "—" or _is_uuid_like_label(_name_s):
                continue
