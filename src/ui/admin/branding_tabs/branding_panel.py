...
def render_tab_branding(sb, tenant_id: str) -> Branding:
    ...
    with st.expander("🎨 Branding (white-label) – opcional", expanded=False):
        company_name = st.text_input(
            "Nome da empresa (no relatório)",
            value=branding_db.get("company_name") or "AgroSafra")
        logo_url = st.text_input(
            "Logo URL (PNG/JPG)",
            value=branding_db.get("logo_url") or "")
        primary_color = st.text_input(
            "Cor primária (hex)",
            value=branding_db.get("primary_color") or "#FFD100")
        accent_color = st.text_input(
            "Cor de destaque (hex)",
            value=branding_db.get("accent_color") or "#7F1D1D")
        footer_note = st.text_input("Rodapé do relatório", value=branding_db.get(
            "footer_note") or "Relatório gerado automaticamente.")
        ...
        st.caption(
            "Dica: para logo, use um link público (Storage do Supabase com bucket público funciona bem).")

    # Retorna Branding com valores do DB (ignora o que foi digitado se não
    # salvo)
