...
def render_admin_revisoes():
    ...
    with tab1:
        ...
        if submitted:
            ...
            # Se datas estão definidas, grava semanas_total automaticamente (pode ser sobrescrito pelo manual)
            semanas_final = 0
            if dt_ini and dt_fim and dt_fim >= dt_ini:
                try:
                    import math
                    dias_total = (dt_fim - dt_ini).days + 1
                    semanas_final = int(math.ceil(dias_total / 7))
                except Exception:
                    semanas_final = 0
            # Se o usuário ajustou manualmente (semanas_total > 0), prioriza
            if semanas_total and int(semanas_total) > 0:
                semanas_final = int(semanas_total)
            if semanas_final > 0:
                payload["semanas_total"] = int(semanas_final)
