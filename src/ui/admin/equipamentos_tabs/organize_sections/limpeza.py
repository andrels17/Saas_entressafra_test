...
def render_limpeza_section(sb, tenant_id: str) -> None:
    ...
    st.caption(
        "Antes de limpar, você pode baixar um CSV com os equipamentos do seu tenant.")
    ...
    with colA:
        ...
        st.caption(
            "Marca `ativo = false` para todos os equipamentos do tenant. Reversível pela aba **Lixeira**.")
        ...
        if st.button(
            ...
            try:
                ...
                st.success(
                    "Todos os equipamentos foram enviados para a lixeira.")
                _rerun()
    ...
    def _pick_canonical(
            rows: list[dict],
            key_created_at: str = "created_at") -> dict:
    ...
    try:
        _using_service_role = callable(get_supabase_service) and (
            sb is not None) and (sb is get_supabase_service())
    except Exception:
    ...
    for g in grps_all:
        ...
        if not k:
            continue
        dep_id = (g.get("departamento_id") if isinstance(
            g, dict) else None) if consider_depto else None
    ...
    if st.button(
        ...
        for _k, items in dep_dups.items():
            ...
            dup_ids = [i.get("id") for i in items if i.get(
                "id") and i.get("id") != canon_id]
            ...
            try:
                ...
                # para debug.
                st.info(
                    "Não foi possível atualizar equip_grupos.departamento_id (coluna inexistente ou bloqueio por permissão/RLS).")
            except Exception:
                st.info(
                    "Não foi possível atualizar equip_grupos.departamento_id (coluna inexistente ou bloqueio por permissão/RLS).")
            ...
            if failed:
                dep_report["soft_disabled"] += _soft_deactivate(
                    "departamentos", failed)

        # 2) Grupos: atualiza vínculos em equipamentos.grupo_id, depois remove
        # duplicados
        ...
        for (_dep_id, _k), items in grp_dups.items():
            ...
            if failed:
                grp_report["soft_disabled"] += _soft_deactivate(
                    "equip_grupos", failed)
        ...
        st.success(
            "Deduplicação concluída. " f"Departamentos: apagados={
                dep_report['hard_deleted']}, desativados={
                dep_report['soft_disabled']}. " f"Grupos: apagados={
                grp_report['hard_deleted']}, desativados={
                    grp_report['soft_disabled']}.")
        _rerun()
