...
def render_ativos_section(sb, tenant_id: str) -> None:
    ...
    with f_col2:
        filtro_grupo = st.selectbox(
            ...
            key="eq_inline_grupo")
    with f_col3:
        filtro_status = st.text_input(
            "Filtrar por status",
            placeholder="Parado, Rodando...",
            key="eq_inline_status")
    ...
    if not rows:
        ...
        st.caption(
            "Edite diretamente na tabela e clique em **Salvar mudanças**. Use **Sel** para ações em lote.")
        ...
        edited = st.data_editor(
            ...
            column_config={
                "sel": st.column_config.CheckboxColumn(
                    "Sel",
                    help="Selecionar para ações em lote"),
                "id": st.column_config.TextColumn(
                    "ID",
                    disabled=True),
                "frota": st.column_config.TextColumn(
                    "Frota",
                    required=True),
        ...
        edited["ano"] = pd.to_numeric(
            edited["ano"], errors="coerce").astype("Int64")
        ...
        selected_ids = list(cur[cur["sel"]].index)
                    ...
                    for _id in changed_ids:
                        ...
                        if payload:
                            sb.table("equipamentos").update(payload).eq(
                                "tenant_id", tenant_id).eq("id", _id).execute()
                            _audit(
                                sb, tenant_id, "update", {
                                    "changes": payload}, equipamento_id=_id)
        ...
        with col2:
            if st.button(
                    ...
                    disabled=(
                        len(selected_ids) == 0),
                    key="eq_inline_soft_del"):
                try:
                    sb.table("equipamentos").update({"ativo": False}).eq(
                        "tenant_id", tenant_id).in_("id", selected_ids).execute()
        ...
        with col3:
            ...
            if st.button(
                ...
                try:
                    ...
                    _audit(
                        sb, tenant_id, "move", {
                            "ids": selected_ids, "grupo_id": dest_gid})
