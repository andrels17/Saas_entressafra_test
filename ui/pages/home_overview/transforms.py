...
def enforce_home_schema(df_like) -> pd.DataFrame:
    ...
    for col in ["pct", "eq_count", "svc_count", "done_steps", "expected_steps"]:
...
def rev_start_end(rev: dict) -> tuple[pd.Timestamp | None, pd.Timestamp | None, int]:
...
def current_week(rev_start: pd.Timestamp | None, semanas_total: int) -> int:
    ...
    try:
        ...
        n = now.tz_localize("UTC") if now.tzinfo is None else now.tz_convert("UTC")
...
def compute_coverage(kdf: pd.DataFrame) -> dict:
    ...
    grupos_com_peso    = int(((kdf["eq_count"] > 0) & (kdf["svc_count"] > 0)).sum())
    scope_w = kdf[(kdf["eq_count"] > 0) & (kdf["svc_count"] > 0)].copy()
    eq_total = int(pd.to_numeric(scope_w.get("eq_count", 0), errors="coerce").fillna(0).sum()) if not scope_w.empty else 0
    eq_done  = int(pd.to_numeric(scope_w.loc[scope_w["pct"] >= 100, "eq_count"], errors="coerce").fillna(0).sum()) if not scope_w.empty else 0
...
def compute_dept_summary(kdf: pd.DataFrame, gid_to_dept: dict) -> tuple[pd.DataFrame, int, int]:
    dsum = calc_dept_kpis(kdf, gid_to_dept)
    if dsum is None or getattr(dsum, "empty", True):
        return pd.DataFrame(), 0, 0
    dep_total = int(len(dsum))
    dep_done  = int((pd.to_numeric(dsum.get("pct", 0), errors="coerce").fillna(0) >= 100).sum())
...
def build_trend_chart_data(sdf: pd.DataFrame) -> pd.DataFrame:
    if sdf.empty:
        return pd.DataFrame()
    g = sdf.groupby("week_number", as_index=False).agg(done=("done_steps", "sum"), exp=("expected_steps", "sum"))
