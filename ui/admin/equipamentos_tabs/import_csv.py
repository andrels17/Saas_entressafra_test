...
def render_import_csv_tab(sb, tenant_id: str):
        admin_block("Importar equipamentos", "Envie o CSV e mapeie colunas com segurança.")
        st.caption("O sistema aceita CSV com separador `,` ou `;`, e tenta detectar encoding automaticamente.")
        st.caption("Campos mínimos: `frota` (obrigatório). Opcional: `modelo`, `ano`, `status`.")
        st.download_button(
        ...
        admin_block("Dicas para CSV do ERP", "Mapeamentos automáticos já suportados pelo importador.")
        ...
        if file is not None:
            ...
            admin_block("Mapeamento de colunas", "Confira e ajuste os campos antes de importar.")
            ...
            map_frota = _sel("Coluna que representa **FROTA** (obrigatório)", "frota")
            map_modelo = _sel("Coluna que representa **MODELO** (opcional)", "modelo")
            map_ano = _sel("Coluna que representa **ANO** (opcional)", "ano")
            map_status = _sel("Coluna que representa **STATUS** (opcional)", "status")
            map_departamento = _sel("Coluna que representa **DEPARTAMENTO** (opcional)", "departamento")
            ...
            if "modelo" in df.columns:
                ...
                df.loc[df["modelo"].str.lower().isin(["nan", "none"]), "modelo"] = ""
            if "status" in df.columns:
                df["status"] = df["status"].astype(str).str.strip()
                df.loc[df["status"].str.lower().isin(["nan", "none"]), "status"] = ""

            if "departamento" in df.columns:
                df["departamento"] = df["departamento"].astype(str).str.strip()
                df.loc[df["departamento"].str.lower().isin(["nan", "none"]), "departamento"] = ""
            ...
            admin_block("Prévia", "Confira os dados antes de confirmar a importação.")
            st.dataframe(df, use_container_width=True, hide_index=True)

            admin_block("Diagnóstico", "Estimativa de novos registros e atualizações.")
            total = len(df)
            existing_frotas = set()
            try:
                ...
                for chunk in _chunked(frotas, 250):
                    ...
                    existing_frotas.update([r["frota"] for r in rows if r.get("frota") is not None])
            ...
            if st.button("Importar / Atualizar", type="primary", use_container_width=True, disabled=(len(df) == 0)):
                # Se vierem departamento/grupo no CSV, criamos/atualizamos automaticamente
                dep_map = {}
                grp_map = {}
                try:
                    if "departamento" in df.columns:
                        dep_map = _ensure_departamentos(sb, tenant_id, df["departamento"].tolist())

                    if "grupo" in df.columns:
                        # Suporta o mesmo nome de grupo em departamentos diferentes.
                        grupo_pairs: list[tuple[str, str | None]] = []
                        if "departamento" in df.columns:
                            for _, rr in df[["grupo", "departamento"]].fillna("").iterrows():
                    ...
                    # Se der erro na criação automática, seguimos importando sem grupo/departamento.
                    dep_map = {}
                    grp_map = {}

                payload = []
                for _, r in df.iterrows():
                    item = {"tenant_id": tenant_id, "frota": str(r["frota"]).strip()}
                    ...
                    if "grupo" in df.columns:
                        ...
                        if gname:
                            ...
                            # fallback: se não achou com dep_id, tenta sem departamento
                            if not gid:
                ...
                try:
                    sb.table("equipamentos").upsert(payload, on_conflict="tenant_id,frota").execute()
                    _audit(sb, tenant_id, "import", {"rows": len(payload)})
                    st.success(f"Importação concluída. Registros processados: {len(payload)}")
                    _rerun()
